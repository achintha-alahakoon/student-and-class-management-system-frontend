...
export default function RegisterTutor() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Always read fresh from localStorage as fallback
    ...
    try {
      await axios.post(
        "http://localhost:8081/api/registration/tutor",
        {
      ...
      setApiSuccess("Tutor registered successfully!");
      setTimeout(() => navigate("/dashboard/admin/tutors"), 1500);
    } catch (err) {
      console.error("Register tutor error:", err.response ?? err);
      ...
      if (err.response?.status === 401) {
        ...
        setApiError(
          err.response.data?.message ?? "Username or NIC already exists.",
        );
      } else {
        setApiError(
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">
            Fill in the details to add a new tutor
          </p>
        </div>
      </div>
