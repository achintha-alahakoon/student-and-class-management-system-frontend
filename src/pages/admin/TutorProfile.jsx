...
import {
  ChevronLeft, Mail, Phone, MapPin, BookOpen,
  CreditCard, Calendar, Edit2, Trash2, CheckCircle, XCircle,
} from "lucide-react";
  ...
  if (!tutor) {
    return (
      <div className="flex flex-col items-center justify-center py-32 text-gray-400">
        ...
        <button onClick={() => navigate("/dashboard/admin/tutors")} className="mt-3 text-sm text-indigo-600">
          Back to tutors
        </button>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-5">
          ...
          <div className="flex-1">
            <div className="flex items-start justify-between flex-wrap gap-3">
              <div>
                ...
                <p className="text-sm text-gray-400 mt-0.5">{tutor.Subject} Tutor</p>
      ...
      <div className="grid grid-cols-3 gap-4">
        <StatCard label="Total Classes"   value={tutor.totalClasses   ?? "—"} color="text-indigo-600" />
        <StatCard label="Total Students"  value={tutor.totalStudents  ?? "—"} color="text-teal-600"   />
        <StatCard label="Attendance Rate" value={tutor.attendanceRate ?? "—"} color="text-green-600"  />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm space-y-4">
          ...
          <InfoRow icon={Calendar}   label="Date of Birth" value={tutor.DOB}     />
          <InfoRow icon={BookOpen}   label="Gender"        value={tutor.Gender}  />
          <InfoRow icon={Calendar}   label="Joined Date"   value={tutor.joinedDate} />
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl mx-4">
            ...
            <p className="text-sm text-gray-500 mb-5">
              This will permanently remove {tutor.FirstName} {tutor.LastName} and all associated data.
            </p>
