...
const TutorsList = ({ onSelectUser }) => {
  ...
  useEffect(() => {
    ...
    const handleSubmit = (e) => {
      ...
      axios.post("http://localhost:8081/api/registration/tutor", formData)
        .then((response) => {
    ...
    const fetchTutorList = () => {
      fetch("http://localhost:8081/api/tutors/tutorslist")
        .then((response) => {
    ...
    const deleteRow = (userId) => {
      console.log(userId);
      axios
        .delete(`http://localhost:8081/api/tutors/${userId}`)
        .then((response) => {
    ...
    <div className="tutors-list">
      ...
      <div className="list-container">
        {tutors.map((tutor, index) => (
          <div className="list" key={index} onClick={() => onSelectUser(tutor.UserID)}>
            <div className="tutor-details">
            <img src={user} alt="user" />
              <h6>{tutor.FirstName} {tutor.LastName}</h6>
            </div>
            <span>{tutor.subject}</span>
            <span className="tutor-todo" onClick={(e) => handlePopoverOpen(e, tutor.UserID)}>:</span>
          </div>
        ))}
      </div>
      ...
      >
        <Box sx={{ width: '100%', maxWidth: 360, bgcolor: 'background.paper' }}>
          <List>
            ...
            <ListItem disablePadding>
              <ListItemButton onClick={() => popup(selectedTutorId)} sx={{ gap: 1, color: 'red' }}>
